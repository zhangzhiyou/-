...
public class LvshenheDao {
    ...
    public void chooseinformantion(Connection con, int id,String agree) throws Exception{
        String sql = "select username,classnumber,applytime1,temail ,unit,phone,fixedphone from lvtables where id="+id;
        ...
        while (rs.next()){
           ...
           String sql1="insert into threetable(username,agree,classroom,applytime,unit,fixedphone,phone) values(?,?,?,?,?,?,?)";
    ...
    public int deleteexam(Connection con, int id) throws Exception{
        String sql = "delete from lvtables where id="+id;
        PreparedStatement pstmt = con.prepareStatement(sql);
    ...
    public int gradeCount(Connection con) throws Exception{//获取总计录数
        String sql = "select count(*) as total from lvtables";
        PreparedStatement pstmt = con.prepareStatement(sql);
        ResultSet rs = pstmt.executeQuery();
        if(rs.next()){
    ...
    public int gradeDelete(Connection con,String delIds)throws Exception{
        String sql = "delete from lvtables where id in("+delIds+")";
