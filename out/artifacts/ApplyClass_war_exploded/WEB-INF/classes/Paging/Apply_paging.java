 ...
 */
public class Apply_paging extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            JSONArray jsonArray= Jsonutil.formatRsToJsonArray(lvshenheDao.gradelist(con,pagebean ));//gradedao.gradelist(con, pagebean)的返回值是ResultSet类型的值
            //Jsonutil.formatRsToJsonArray（）参数类型是ResultSet类型的 返回 JSONArray类型。把gradedao.gradelist(con, pagebean)转换成json,
            int total=lvshenheDao.gradeCount(con);//获取总数
